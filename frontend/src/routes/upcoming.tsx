...
import { getUpcomingMovies } from '@/lib/movies.server'
import { z } from 'zod'
...
export const Route = createFileRoute('/upcoming')({
  ...
  loader: async ({ deps }) => {
    const data = await getUpcomingMovies({ data: { page: deps.page } })
    return { 
      movies: data.results,
      page: data.page,
        ...
        <div className="my-8">
          ...
          {movies && movies.length > 0 ? (
            <>
              ...
              <Pagination page={page} totalPages={totalPages} path="/upcoming" />
            </>
          ) : (

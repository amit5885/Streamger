...
import { getTrendingMovies } from '@/lib/movies.server'
import { z } from 'zod'
...
export const Route = createFileRoute('/trending')({
  ...
  loader: async ({ deps }) => {
    const data = await getTrendingMovies({ data: { page: deps.page } })
    return { 
      movies: data.results,
      page: data.page,
        ...
        <div className="my-8">
          ...
          {movies && movies.length > 0 ? (
            <>
              ...
              <Pagination page={page} totalPages={totalPages} path="/trending" />
            </>
          ) : (
